...
- Support light, dark, and system themes. The theme toggle stores the preference in local storage and applies the `dark` class to the document root.
...
- A route's existence does not imply it belongs in navigation. The Values route is currently a stub and its navigation entry is disabled.
